...
/**
 * Find USB COM ports matching certain driver descriptions. Useful for singling out ports
 * based on device type (e.g. Prolific, FTDI, Maple)
 */
public class PortFinder {
	 ...
	 */
	private static final Pattern WIN_USB_ID = Pattern.compile("VID_([0-9a-f]{4}).+" +
		"PID_([0-9a-f]{4})", Pattern.CASE_INSENSITIVE);

	/**
	 * Finds orphaned ports and sets them straight with HardwareSerial instances.
	 * 
	 * @param have the list of ports that have a match
	 * @param want all ports, including the ones not matched
	 */
	private static void cleanupExtras(final List<Serial> have,
	...
	/**
	 * Cross-platform method to enumerate all serial ports on the system. This method will
	 * work more often (e.g. on mac) but will not fill in extended information (all will come
	 * back as HardwareSerial instances). Use when getPortList() throws an exception.
	 *
	...
	/**
	 * Enumerate the serial ports on a Unix/Linux machine given the list of active COM ports.
	 *
				...
				for (File devDir : usbDevice.listFiles())
					if (devDir.isDirectory() && devDir.getName().contains(usbId))
						...
						for (String port : coms) {
							...
							if (new File(devDir, pname).exists() ||
									new File(pfile, pname).exists()) {
	 ...
	 * @param overrideName null to use the port's stated name, otherwise this will be used
	 */
	private static void enumWindowsCom_(final String key, final List<Serial> out,
			final List<String> coms, final String overrideName) {
		String[] keys;
		...
		for (String usb : keys)
			try {
				...
				final String[] subKeys = Advapi32Util.registryGetKeys(WinReg.HKEY_LOCAL_MACHINE,
					path);
				for (String sk : subKeys) {
					// Friendly name
					String name = Advapi32Util.registryGetStringValue(WinReg.HKEY_LOCAL_MACHINE,
						path + "\\" + sk, "FriendlyName");
					// Strip off the last () device
					final int pos = name.lastIndexOf('(');
					if (pos > 0)
						name = name.substring(0, pos).trim();
					// Device Parameters\PortName has the PORT #
					final String com = Advapi32Util.registryGetStringValue(
						WinReg.HKEY_LOCAL_MACHINE, path + "\\" + sk + "\\Device Parameters",
						"PortName");
					for (String pluggedIn : coms)
						...
						if (pluggedIn.equalsIgnoreCase(com)) {
							...
							if (m.find()) {
								...
								// FTDI comes back as "USB Serial Port", not that helpful
	...
	/**
	 * Enumerates the serial ports on a Windows machine given the list of active COM ports.
	 *
	 * @param coms the communication port names available
	 * @return the serial ports found
	 */
	...
	/**
	 * Finds all serial ports matching the specified ID. If none are found, returns null.
	 *
	...
	public static List<Serial> findByID(final String id, final List<Serial> input) {
		...
		for (Serial ser : input)
			...
			if (ser.getID().toUpperCase().contains(m) ||
					ser.getName().toUpperCase().startsWith(id))
				out.add(ser);
		return out;
	}
	/**
	...
	private static String readOneLine(final File input) {
		try {
			// Read in one line from the file; if it's empty, it gets NULL which is intended
	...
	public static interface Serial {
		 ...
		 *
		 * @return the communication port identifier for opening a serial connection
		 */
		...
		/**
		 * Gets the serial port identifier. For hardware ports, this will return the same value
		 * as getComPort(). USB and other extended devices will return a more specific
		 * identifier good for identifying devices.
		 *
		 * @return the port ID
		 */
		public String getID();
